...
namespace OOP14
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            dogList.Sort(); // sort list by ID

            SaveToFile(dogList.ToArray<Dog>()); // serialization list
            var puppiesFromFile = LoadFromFile();   // deserialization
            foreach (var item in puppiesFromFile)
            {
                System.Console.WriteLine(item); // output elements of an array gotten from a file
            }
        }

        public static void SaveToFile(Dog[] dogs)
        {
            BinaryFormatter binformat = new BinaryFormatter();  // instance is needed for binary writing files
            using (FileStream fileStream = new FileStream("zoostore.txt", FileMode.Append, FileAccess.Write))
            {
                binformat.Serialize(fileStream, dogs);  // write dog array into the file
            }
        }
        public static Dog[] LoadFromFile()
        {
            BinaryFormatter binformat = new BinaryFormatter();  // instance is needed for binary reading files
            using (FileStream fileStream = new FileStream("zoostore.dat", FileMode.OpenOrCreate, FileAccess.Read))
            {
                return (Dog[])binformat.Deserialize(fileStream);    // take dog array from the file
            }
        }
    }
}
